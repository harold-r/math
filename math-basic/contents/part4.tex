...
虽然$360^{\circ}$在日常生活和航海等机器方便，但在现代高级数学和物理学（如微积分）中，科学家们更倾向于使用弧度制（Radian）。在弧度制中，一个完整的圆被定义为 2π，因为它是纯粹基于圆的半径和弧长计算出来的自然度量，不再依赖人类主观规定的数字。
...
\begin{minipage}{0.7\textwidth}
  \textbf{倍角：}比如一个角是另一个角得两倍，那么往外延伸大角，出来一个等腰，得出$1/2$角，这个常见。如果做大角得角平分线或者双倍角往往无从下手。如右图，已知$AB$的长，$M$是$BC$的中点，求$DM$长。作出辅助线后，可知$DM=EC/2-BC/2=AB/2$。当然，用三角函数也可以求出，何其难！
  \end{minipage}
\begin{minipage}{0.3\textwidth}
  \centering
  \begin{tikzpicture}[scale=1]
...
\begin{exercise}
在初中知识下：如果$\sin A=\sin B$，$A,B$是锐角，求证$A=B$
\end{exercise}
\begin{solution}
