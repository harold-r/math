...
事实上，《几何原本》的命题都建议去自己去证明一下，会很有裨益。

圆被定义为360度是人类历史、天文学和数学实用性共同演化的结果。

古巴比伦发明并使用六十进制，不仅讲圆周划分为$6$个$60^{\circ}$，还进一步把小时划分为$60$分钟，分钟细分为$60$秒，这套系统被古希腊天文学家和数学家采纳并标准化。$360$在数学上是一个高度的合成数，有极多的因数，便于整除，从$1$到$10$除了$7$，都可以被整除。如果圆被定义为$100$度，那么三分就是$33.333\ldots$，选择$360^{\circ}$也就是更容易被平分。

虽然$360^{\circ}$在日常生活和航海等机器方便，但在现代高级数学和物理学（如微积分）中，科学家们更倾向于使用弧度制（Radian）。在弧度制中，一个完整的圆被定义为 $2\pi$，因为它是纯粹基于圆的半径和弧长计算出来的自然度量，不再依赖人类主观规定的数字。

为什么一天是$24$个小时？人们最容易取得的计算工具是手指，一只手，去除大拇指，4根手指有12个指节，中国的“子丑寅卯 辰巳午未 申酉戌亥”也是12个。
...
面积公式：$S=\frac{1}{2}ab\sin\theta$，$a,b,\theta$分别为两边以及两边的夹角。
...
\begin{minipage}{0.8\textwidth}
  全等除了AAA(相似)和ASS(屁股)(如右图)，其余(SSS, SAS, AAS/ASA/SAA)都是全等。

  ASS下如果A是钝角呢？从正弦定理来看，如果A是锐角，那么另一边对应的角有可能是锐角，也可能是钝角。如果A是钝角，那么另外一个角就必须是锐角，也就确定了，就是全等了。
\end{minipage}
...
\end{minipage}
  ...
  对于直角三角形全等判定，因为多了一个特定的直角，由勾股定理，已知两边也就确定了第三边，ASS也可以，也就是HL。

\subsection{相似}
几何原本第六卷，相似三角形的定义：如果两个三角形对应的角相等，且对应边的比相等，则这两个三角形相似。

命题\uppercase\expandafter{\romannumeral6}.2：平行于三角形底边的直线截得的三角形两边的线段成比例。\\
命题\uppercase\expandafter{\romannumeral6}.3：三角形角平分线截得的底边线段之比等于两侧边之比。\\
命题\uppercase\expandafter{\romannumeral6}.4：(AAA)如果两个三角形各角相等，那么等角所对应的边成比例。\\
命题\uppercase\expandafter{\romannumeral6}.5：如果两个三角形的边成比例，那么各边所对的角相等。\\
命题\uppercase\expandafter{\romannumeral6}.6：(SAS)如果两个三角形有一个角相等，且夹该等角的边成比例，那么这两个三角形各角对应相等。\\
命题\uppercase\expandafter{\romannumeral6}.7：(ASS带限定条件)如果两个三角形有一个角相等，且夹另外一个角的边成比例，其余的角要么都小于要么都不小于一直角，则这两个三角形是等角的，成比例的边所夹的角相等。\\
命题\uppercase\expandafter{\romannumeral6}.8：直角三角形

命题\uppercase\expandafter{\romannumeral6}.21：(相似的传递性)与同一直线形相似的图形也彼此相似
...
相似的比例有两种写法：$\frac{AB}{AC}=\frac{A'B'}{A'C'},  \text{or}\;\; \frac{AB}{A'B'}=\frac{AC}{A'C'}=\frac{BC}{B'C'}(=k)$，注意微妙的不同，前者可以写为一个$ AB:AC:BC=DE:DF:EF$。几何原本里，用的是前者。如果有面积比，周长比，则后者较为直观。
...
\begin{minipage}{0.5\textwidth}
  ...
  \begin{tikzpicture}
    ...
    \tkzLabelAngles[pos=0.7, red, scale=0.8](C,A,E A,C,C'){$\alpha$}
    \tkzLabelAngles[pos=0.7, red, scale=0.8](E,A,A' C,C',A){$\beta$}
...
\phantom{xxxxx}推论：同圆中或等圆中，同弧对等角，等弧对等角，等角对等弧\\
命题\uppercase\expandafter{\romannumeral3}.31：半圆上的圆周角是直角；大于半圆的圆周角是锐角，小于半圆的圆周角是钝角\\
命题\uppercase\expandafter{\romannumeral3}.32：弦切角定理\\
...
\subsection{点到直线的距离}
若点$P(x_0,y_0)$, 直线$Ax+By+C=0$，则距离：$\displaystyle  d=\frac{|Ax_0+By_0+C|}{\sqrt{A^2+B^2}}$
...
\subsection{平行线之间的距离}
假设两条直线分别为$Ax+By+C_1=0, \; Ax+By+C_2=0$，假设$P(x_0,y_0)$在第一条在直线上，那么$P$到另外一条直线上的距离就是:$ d=\frac{|Ax_0+By_0+C_2|}{\sqrt{A^2+B^2}}$,
...
思考：如果三角形内找一点，使得这个点到任意两个三角形顶点的角度为$120^{\circ}$
...
$AB=2r\sin\theta$，只需找出最小外接圆半径即可， while $h\le r+r\cos\theta \Rightarrow r \ge h/(1+\cos\theta)$ or $h+r\cos(90^{\circ}-\theta/2) \le r \Rightarrow r\ge h/(1-\sin \frac{\theta}{2})$。最小值的情况下，都是$OC \perp AB$，也就是$OC$平分定角，也就是$CA=CB$，$\triangle CAB$是等腰三角形。
...
\textbf{中点：}1.大概率跟中位线有关，2.还有倍长中线(构造全等，得出平行)

\begin{minipage}{0.7\textwidth}
  \textbf{倍角：}1. 如右图，大角一分为二，不是作角平分线，而是向外作等腰三角形；2，有时候可以作对称小角的翻折。右图，已知$AB$的长，$M$是$BC$的中点，$AD\perp BC$，求$DM$长。作出辅助线后，可知$DM=EC/2-BC/2=AB/2$。当然，用三角函数也可以求出，何其曲折！
  \end{minipage}
\begin{minipage}{0.3\textwidth}
  \centering
  \begin{tikzpicture}[scale=1]
    ...
    \tkzDefPointsBy[rotation=center C angle -30](B){A''}
    \tkzInterLL(B,A')(C,A'') \tkzGetPoint{A}
    \tkzDefPointsBy[rotation=center B angle 120](A){E}

    \tkzMarkAngles[size=.5, mksize=2pt](A,C,B)
    \tkzMarkAngles[size=.2,  mksize=2pt](C,B,A)
    \tkzLabelAngle[pos=.7,  font=\scriptsize, red](A,C,B){$\theta$}
    \tkzLabelAngle[pos=.4,  font=\scriptsize, red](C,B,A){$2\theta$}
    \tkzDefPointOnLine[pos=0.5](B,C)  \tkzGetPoint{M}
    \tkzDefPointsBy[projection=onto B--C](A){D}
    \tkzMarkRightAngles[scale=0.8](A,D,C)
    \tkzDrawSegments(A,B A,C B,C A,D)
    \tkzDrawPoints(M)
    \tkzDrawSegments[dashed](B,E A,E)
    \tkzLabelPoints[above](A)
    \tkzLabelPoints[](D,B,E,C,M)
  \end{tikzpicture}
\end{minipage}

\begin{minipage}{0.7\textwidth}
  \textbf{截长补短、位置角：} \\比如出现求证$AB=BC+CF$之类，还有特殊，如右图：\\
  等边三角形$ABC$，腰上动点$E,F$, $EB=2AF$，做等边$EFG$，求$\angle FCG$的度数。 围绕内部的等边作全等，可以延长$BA$至$F'$, $AF'=AF$，那么$EF'=FC$，$\triangle EFF' \cong \triangle FGC$.当然，也可以截取，$CA$上，取一点$E'$，使得$CE'=AF$，那么$EF'=AE$。

  全等需要一个\textbf{等角}，也是常用，就是$\angle GFC+ \angle AFE=120^{\circ} = \angle AEF + \angle AFE \Rightarrow \angle AEF= \angle GFC$
\end{minipage}
\begin{minipage}{0.3\textwidth}
  \centering
  \begin{tikzpicture}[scale=1]
    \tkzDefPoints{0/0/B, 3/0/C}
    \tkzDefPointsBy[rotation=center B angle 60](C){A}
    \tkzDefPointOnLine[pos=0.3](B,A)  \tkzGetPoint{E}
    \tkzDefPointOnLine[pos=0.15](A,C)  \tkzGetPoint{F}
    \tkzDefPointsBy[rotation=center E angle -60](F){G}
    \tkzMarkAngles[size=.5, mark=|, mksize=2pt](F,E,A)
    \tkzMarkAngles[size=.5, mark=|, mksize=2pt](G,F,C)
    \tkzDrawSegments(A,B A,C B,C E,F C,G)
    \tkzDrawPolygons(E,F,G)
    \tkzLabelPoints[above](A)
    \tkzLabelPoints[left](B,E)
...
还有一个就是出现$2$倍的长度，而且还是直角三角形的直角边，可以直接作翻折。

\hrulefill

\begin{exercise}
在初中知识下：$A,B$是锐角，如果$\sin A=\sin B$，求证$A=B$
\end{exercise}
\begin{solution}
\textbf{【解析】}构造直角三角形， 证相似
\end{solution}
...
\begin{exercise}
  \begin{minipage}{0.3\textwidth}
    \centering
    \begin{tikzpicture}[scale=1]
      \tkzDefPoints{0/3/A, 0/0/B, 3/0/C}
      \tkzDefPointOnLine[pos=0.5](A,B)  \tkzGetPoint{D}
      \tkzDefPointsBy[projection=onto C--D](B){E}
      \tkzInterLL(A,C)(B,E) \tkzGetPoint{F}
      \tkzMarkRightAngles[scale=0.8](D,E,B C,B,A)
      \tkzDrawSegments(A,B A,C B,C D,F B,F C,D)
      \tkzLabelPoints(E)
      \tkzLabelPoints[above](A)
  ...
  \end{minipage}
  \begin{minipage}{0.7\textwidth}
    如图：等腰直角三角形$ABC$，$\angle B$为直角，$AB=BC$，$D$为$AB$的中点，连接$CD$，从$B$作$CD$的垂线，$E,F$分别为垂足和与$AC$的交点。 求证：$\angle ADF=\angle BDC$\\
    变形：求证 $3AF=AC$。
  \end{minipage}
\end{exercise}
\begin{solution}
  \begin{minipage}{.25\textwidth}
    \centering
    \begin{tikzpicture}[scale=1]
      \tkzSetUpAxis[tickwd=0pt, ticka=0pt, tickb=0pt]
      \tkzDefPoints{0/3/A, 0/0/B, 3/0/C, 3/3/G}
      \tkzDefPointOnLine[pos=0.5](A,B)  \tkzGetPoint{D}
      \tkzDefPointsBy[projection=onto C--D](B){E}
      \tkzInterLL(A,C)(B,E) \tkzGetPoint{F}
      \tkzInterLL(A,G)(B,E) \tkzGetPoint{J}
      \tkzMarkRightAngles[scale=0.8](D,E,B C,B,A)
      \tkzDrawSegments(A,B A,C B,C D,F B,F C,D)
      \tkzLabelPoints(E)
      \tkzLabelPoints[left](A,B,D)
  ...
  \begin{minipage}{.25\textwidth}
    ...
    \begin{tikzpicture}[scale=1]
      ...
      \tkzMarkRightAngles[scale=0.8](D,E,B C,B,A)
      \tkzDrawSegments(A,B A,C B,C D,F B,F C,D)
      \tkzDrawSegments[dashed](A,G C,G F,J)
      \tkzLabelPoints(E)
  ...
  \begin{minipage}{.25\textwidth}
    ...
    \begin{tikzpicture}[scale=1]
      ...
      \tkzInterLL(A,C)(B,E) \tkzGetPoint{F}
      \tkzInterLL(A,G)(B,E) \tkzGetPoint{J}
      \tkzDrawSegments(A,B B,C B,F C,D A,G C,G F,J)
      \tkzLabelPoints(E)
      \tkzLabelPoints[above](A,J)
      \tkzLabelPoints[left](B,D)
      \tkzLabelPoints[right](C,G)
  ...
  \begin{minipage}{.25\textwidth}
    ...
    \begin{tikzpicture}[scale=1]
      \tkzDefPoints{0/3/A, 0/0/B, 3/0/C}
      \tkzDefPointOnLine[pos=0.5](A,B)  \tkzGetPoint{D}
      \tkzDefPointOnLine[pos=0.33](C,A)  \tkzGetPoint{G}
      \tkzDefPointsBy[projection=onto C--D](B){E}
      \tkzInterLL(A,C)(B,E) \tkzGetPoint{F}
      \tkzMarkRightAngles[scale=0.8](D,E,B C,B,A)
      \tkzDrawSegments(A,B A,C B,C D,F B,F C,D)
      \tkzDrawSegments[dashed](B,G)
      \tkzLabelPoints(E)
      \tkzLabelPoints[above](A)
      \tkzLabelPoints[left](B,D)
...
\textbf{【解析】}Honestly, 一开始，无从下手，主要是那个垂直。等腰直角三角形定，$AB$中点定，$AE,AF$定，所有的点都是定的，角度也是定的，这两个角度相等，而且角度的值也是定的。最后不得已建立直角坐标系，用三角函数。

上图左：建立直角坐标系，设直角边为$2$, Line $AC: x+y=2$; Line $CD: y=-\frac{1}{2}x$; Line $BF: y=2x$，可以求出$F(\frac{2}{3}, \frac{4}{3})$，$\tan \angle ADF=\frac{F_x-D_x}{F_{y}-D_y}=2=\tan \angle BDC$.

建系虽能很快解决问题，但是总感觉差点意思，在初中知识下算是不合适的解答。所以要用平面几何。因为$D$是中点，所以，可以作平行线，但是角度也不好转换，全等不太好找。所以，就考虑扩展图形，如上图-2：延长$BF$后，就出现了一对貌似全等的图形$\triangle AFJ, \triangle AFD$，而垂线$BE$恰好可以用来证明$\triangle BDC \cong \triangle AJB$。

扩展成正方形后，产生一个很有趣的问题，如上图-3：如果$D,J$分别正方形两个临边的中点，$CD$和$BJ$什么关系？事实上，$D,J$只要满足$BD=AJ$，那么两条线段都是垂直关系。

回到原题，一般碰到中点，辅助线作平行线，我们试一下。其实这个辅助线first sight就是跟$BE,CD$的垂直有点割裂。如果不是看到书本，也想不到这种思路的。假设所证成立，那么$\angle ADF = \angle FBC$，就有$\triangle ADF \sim CBF$，得到$\angle AFD = \angle CFB$，可以推出$\angle BGC = \angle BFA$，得到$CG=AF$，那么中位线定理已知$AF=FG$，所以，$F,G$是$AC$的三等分点，还有就是，角度上的收获，$C$点射向$AB$的中点，镜面$AB$反射到$F$点，镜面$AC$再反射到$B$点，意外收获。但是如果用这个辅助线去做，还是不简单。书中取$AC$的三等分点$F', G'$，证明$F,F'$重合，用三角函数。
...
\hrulefill
 \begin{exercise}
   \begin{minipage}{0.3\textwidth}
     \centering
     \begin{tikzpicture}[scale=1]
       \tkzDefPoints{0/1/A, 0/0/B, 2.5/0/C}
       \tkzDefPointsBy[rotation=center A angle 60](C){D}
       \tkzDefPointsBy[rotation=center A angle 30](B){E'}
       \tkzInterLL(A,E')(B,C) \tkzGetPoint{E}
       \tkzMarkRightAngles[scale=0.8](A,B,C)
       \tkzDrawSegments(A,B B,C A,C C,D A,D D,E A,E)
       \tkzLabelPoints[above](A)
       % \tkzLabelPoints[left](B)
       \tkzLabelPoints[right](C,D)
       \tkzLabelPoints(B,E)
   ...
   \end{minipage}
   \begin{minipage}{0.7\textwidth}
     如图：直角三角形$ABC$，$\angle ABC=90^{\circ}$，$AC=AD$，$\angle CAD = 2\angle BAE$，下面正确的结论是：\textcircled{\footnotesize 1}$AC \perp DE$, \textcircled{\footnotesize 2}$\angle BCA=\angle EDA$, \textcircled{\footnotesize 3}如果$CD\parallel AB$那么$\angle EAD=90^{\circ}$，\textcircled{\footnotesize 4}$DE=CE+2BE$
   \end{minipage}
\end{exercise}
\begin{solution}
  \begin{minipage}{0.7\textwidth}
    \textbf{【解析】}如果按照1234的顺序去做，事倍功半。比如1毫无头绪，那就看2，乍一看$A,E,C,D$四点共圆，对角互补可求证，然后我们再做3、4，那么到4后，发现，之前的也没必要去那么麻烦去做。$\triangle E'AC \cong EAD$，一切迎刃而解。所以，对于填空或选择题来说，这类题型一定要从大局出发。
  \end{minipage}
  \begin{minipage}{0.3\textwidth}
    \centering
    \begin{tikzpicture}[scale=1]
      ...
      \tkzDefPointsBy[rotation=center A angle 30](B){E'}
      \tkzInterLL(A,E')(B,C) \tkzGetPoint{E}
      \tkzDefPointOnLine[pos=2](E,B)  \tkzGetPoint{E'}
      \tkzMarkRightAngles[scale=0.8](A,B,C)
      \tkzDrawSegments(A,B B,C A,C C,D A,D D,E A,E)
      \tkzDrawSegments[dashed](B,E' A,E')
      \tkzLabelPoints[above](A)
 ...
 \begin{exercise}
   \begin{minipage}{0.3\textwidth}
     ...
     \begin{tikzpicture}[scale=1]
       ...
       \tkzDefPointBy[rotation=center B angle 60](C) \tkzGetPoint{A}
       \tkzDefPointOnLine[pos=1.5](B,A)  \tkzGetPoint{D}
       \tkzDefPointBy[rotation=center D angle 120](C) \tkzGetPoint{E}
       \tkzDrawSegments(A,B A,C B,C C,D D,E)
       \tkzDrawSegment[add= 0 and 1.2](B,A)
       \tkzDefPointOnLine[pos=.5](B,E)  \tkzGetPoint{F}
       \tkzDrawSegments(A,B A,C B,C C,D B,E D,F)
       \tkzDrawSegment[add= 0 and 0.7](B,A)
       \tkzLabelPoints[left](A,B,D)
   ...
   \end{minipage}
   \begin{minipage}{0.7\textwidth}
     如图：三角形$ABC$为等边三角形，$D$为$BA$延长线一点，连接$CD$，$DC$绕$D$逆时针旋转$120^{\circ}$，得到$E$点，$F$为$BE$的中点，求$DF$和$DA$的数量关系，另，$DF$和$AC$平行吗？
   \end{minipage}
 \end{exercise}
 \begin{solution}
   \textbf{【解析】}乍一看，应该是$2$倍的关系，$\angle CDE=120^{\circ} \Rightarrow \angle EDA'=\angle ACD$，所以需要一个全等，又$F$是中点，所以，中位线定理再合适不过了，所以$D$应该是另一个边的中点，需要延长$BD$使得，$DB'=DB$，又需要一个全等，所以，需要一个$A'$使得$DA'=AB$，那么$\triangle A'DE \cong ACD$，$\triangle A'B'E$是等边三角形。

   但是中位线定理之前，还有一个很重要的点，就是倍长中线，如下右图，延长$DF$使得$DF=FG$，那么$DBGE$就是一个平行四边形，直接找含有$DG,AD$的三角形全等比较困难，只能曲线救国，$\triangle BAG \cong \triangle CAD$，$\triangle ADG$ 是等边三角形。
   ...
   \begin{minipage}{0.5\textwidth}
     ...
     \begin{tikzpicture}[scale=1]
       ...
       \tkzDefPointBy[rotation=center B angle 60](C) \tkzGetPoint{A}
       \tkzDefPointOnLine[pos=1.5](B,A)  \tkzGetPoint{D}
       \tkzDefPointOnLine[pos=2.5](B,A)  \tkzGetPoint{A'}
       \tkzDefPointOnLine[pos=3](B,A)  \tkzGetPoint{B'}
       \tkzDefPointBy[rotation=center D angle 120](C) \tkzGetPoint{E}
       \tkzDrawSegments(A,B A,C B,C C,D D,E)
       \tkzDrawSegment[add= 0 and 1.2](B,A)
       \tkzDefPointOnLine[pos=.5](B,E)  \tkzGetPoint{F}
       \tkzDrawSegments(A,B A,C B,C C,D B,E D,F D,B')
       \tkzDrawSegments[dashed](B',E A',E)
       \tkzDrawSegment[add= 0 and 0.7](B,A)
       \tkzLabelPoints[left](A,B,D,A',B')
       \tkzLabelPoints[right](E,F)
       \tkzLabelPoints(C)
     \end{tikzpicture}
   \end{minipage}
   \begin{minipage}{0.5\textwidth}
     ...
     \begin{tikzpicture}[scale=1]
       ...
       \tkzDrawSegment[add= 0 and 1.2](B,A)
       \tkzDefPointOnLine[pos=.5](B,E)  \tkzGetPoint{F}
       \tkzDefPointOnLine[pos=2](D,F)  \tkzGetPoint{G}
       \tkzDrawSegments(A,B A,C B,C C,D B,E D,F)
       \tkzDrawSegments[dashed](F,G E,G B,G A,G)
       \tkzDrawSegment[add= 0 and 0.7](B,A)
       \tkzLabelPoints[left](A,B,D)
       \tkzLabelPoints[right](E,F,G)
       \tkzLabelPoints(C)
     \end{tikzpicture}
   \end{minipage}
 \end{solution}
