...
\textbf{素数定理}, 又称作质数定理, prime number theorem，是素数分布理论的中心定理，是关于素数个数问题的一个命题：\todo{...}
...
被4整除：\\
The last two digits must be divisible by 4. Tips\footnote{$A=\overline{a_{n}a_{n-1}ldots a_3} \times 100 + \overline{a_2 a_1}$}，后者是关键
...
$a^3+b^3+c^3=(a+b+c)^3+3(a+b+c)(-ab-ac-bc)+3abc$，对称多项式\\
...
\]
\begin{gather*}
  y^n-y^n=(x-y)(x^{n-1}+x^{n-2}y+x^{n-3}y^2+\cdots + x^2y^{n-3}+xy^{n-2}+y^{n-1})\\
  x^n+y^n=(x+y)(x^{n-1}-x^{n-2}y+x^{n-3}y^2-\cdots +x^2y^{n-3}-xy^{n-2}+y^{n-1}y) \cdots \text{n is odd}
\end{gather*}
$x^4+y^4=(x^2+y^2)^2-2(xy)^2$

$x^4+4y^4=x^4+4x^2y^2+4y^2-4x^2y^2$, named as ``Sophie Germain's identity''
